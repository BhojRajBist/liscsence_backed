...
class RandomQuestionsAPIView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        for chapter in random_chapters:
            chapter_records_weightage_1 = civilQuestions.objects.filter(chapter=chapter, weight=1)
        ...
        serializer = civilQuestionSerializers(all_records, many=True)

        return Response(serializer.data, status=status.HTTP_200_OK)
    
#fundamental of surveying
class RandomQuestionsByChapterCodeView(generics.ListAPIView):
    serializer_class = civilQuestionSerializers
    ...
    def get_queryset(self):
        ...
        filtered_questions = civilQuestions.objects.filter(chapter=chapter_obj)
